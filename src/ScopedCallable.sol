    ...
    IERC7786Receiver
{
    ...
    mapping(bytes4 selector => bool supported) internal _supportedAttributes;
    ...
    ) external payable virtual returns (bytes memory response) {
    ...
    function fillResponsesIn(
        ...
        require(
            requestHashes.length == responses.length,
            "requestHashes and responses must have the same length"
        );
        require(
            chainIds.length == requestHashes.length,
            "chainIds and requestHashes must have the same length"
        );
        for (uint256 i = 0; i < requestHashes.length; i++) {
            _updateResponsesInboxValue(
                chainIds[i],
                requestHashes[i],
                responses[i]
            );
            _updateRollingHash(
                chainIds[i],
                keccak256(responses[i]),
                RollingHashType.RESPONSES_IN
            );
        }
    }
    ...
    function sendMessage(
        ...
        for (uint256 i = 0; i < attributes.length; i++) {
            if (!_supportedAttributes[bytes4(attributes[i])]) {
                revert IERC7786GatewaySource.UnsupportedAttribute(bytes4(attributes[i]));
            }
        }
        bytes32 requestHash = _getTransactionHash2(
            destinationChain,
            receiver,
            payload,
            attributes
        ...
        _updateRollingHash2(
            destinationChain,
            requestHash,
            RollingHashType.REQUESTS_OUT
        );

        emit MessagePosted(requestHash, destinationChain, receiver, payload, msg.value, attributes);

        // We return a storage location so the calling function can read the response while remaining compatible with the ERC7786 spec
        bytes32 responseLocation = _calcStorageKey(
            RollingHashType.RESPONSES_IN_VALUES,
            destinationChain,
            requestHash
        );
    ...
    function executeMessage(
        ...
        _updateRollingHash2(
            sourceChain,
            requestHash,
            RollingHashType.REQUESTS_IN
        );

        // Decode the request from the payload
        ScopedRequest memory request = abi.decode(payload, (ScopedRequest));

        // Execute local call
        (bool success, bytes memory result) = request.to.call{ // todo use the receiver address
            gas: request.gasLimit,
            value: request.value
        }(request.data);

        if (!success) {
            revert IScopedCallable.CrossChainCallFailed();
        }
    ...
    // todo add setter for supportedAttributes
    function supportsAttribute(bytes4 selector) external view returns (bool) {
        return _supportedAttributes[selector];
    }

    // View functions

    /// @inheritdoc IScopedCallable
    function rollingHashesEqual(
        uint256 chainId_,
        RollingHashes memory other
    ) external view returns (bool) {
        return _rollingHashesEqual(chainId_, other);
    }

    /// @inheritdoc IScopedCallable
    function readRollingHash(
        uint256 chainId_,
        RollingHashType rollingHashType
    ) external view returns (bytes32) {
        return _readRollingHash(chainId_, rollingHashType);
    }

    /// @inheritdoc IScopedCallable
    function readResponsesInboxValue(
        uint256 chainId_,
        bytes32 requestHash
    ...
    /// @inheritdoc IScopedCallable
    function getRollingHashes(
        uint256 chainId_
    ) external view returns (RollingHashes memory) {
    ...
    function _scopedCall(
        ...
        bytes32 requestHash = _getTransactionHash(
            _chainId,
            targetChainId,
            from,
            nonce,
    ...
    function _handleScopedCall(
        ...
        // Execute local call
        (bool success, bytes memory result) = request.to.call{
            gas: request.gasLimit,
            value: request.value
        }(request.data);
    ...
    function _rollingHashesEqual(
        ...
        return
            rollingHashes.requestsOut == other.requestsIn &&
            rollingHashes.requestsIn == other.requestsOut &&
            rollingHashes.responsesOut == other.responsesIn &&
            rollingHashes.responsesIn == other.responsesOut;
    }

    function _getRollingHashes(
        uint256 chainId_
    ) internal view returns (RollingHashes memory) {
        return
            RollingHashes({
                requestsOut: _readRollingHash(
                    chainId_,
                    RollingHashType.REQUESTS_OUT
                ),
                requestsIn: _readRollingHash(
                    chainId_,
                    RollingHashType.REQUESTS_IN
                ),
                responsesOut: _readRollingHash(
                    chainId_,
                    RollingHashType.RESPONSES_OUT
                ),
                responsesIn: _readRollingHash(
                    chainId_,
                    RollingHashType.RESPONSES_IN
                )
            });
    ...
    function _getTransactionHash2(
        ...
        return
            keccak256(
                bytes.concat(
                    bytes32(keccak256(abi.encodePacked(destinationChain))),
                    bytes32(keccak256(abi.encodePacked(receiver))),
                    bytes32(keccak256(payload)),
                    bytes32(keccak256(abi.encode(attributes)))
                )
            );
    }
    ...
    function _calcStorageKey(
        RollingHashType rollingHashType,
        string calldata chain,
        bytes32 requestHash
    ) internal pure returns (bytes32) {
        return keccak256(abi.encode(rollingHashType, chain, requestHash));
    }

    // Updates and writes the rolling hash to transient storage
    // Note the getCompat function does not use transient storage on an L2s
    function _updateRollingHash2(
        string calldata chain, // CAIP-2 chain identifier
        bytes32 newHash,
        RollingHashType rollingHashType
    ) internal {
        // Each chainId has its own unique transient storage slot
        bytes32 key = keccak256(abi.encodePacked(rollingHashType, chain));
    ...
    function _updateResponsesInboxValue(
        uint256 chainId_,
        bytes32 requestHash,
        bytes memory result
    ) internal {
    ...
    function _readResponsesInboxValue(
        uint256 chainId_,
        bytes32 requestHash
    ) internal view returns (bytes memory) {
        // Each chainId has its own unique transient storage slot
        // todo use _calcStorageKey
        bytes32 key = keccak256(
            abi.encode(
                RollingHashType.RESPONSES_IN_VALUES,
                chainId_,
